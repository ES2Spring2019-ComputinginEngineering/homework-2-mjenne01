...
def intersectionoftwolines_x(m1, b1, m2, b2):
    ...
    # find x coordinate by doing (b2-b1)/(m1-m2)
...
def intersectionoftwolines_y(m1, b1, m2, b2):
    ...
    # y = (m1 * x) + b1 and y = (m2 * x) + b2 intersect.
    # y coordinates same if (m1*x)+b1 = (m2*x)+b2
    y = ((b2 * m1) - (b1 * m2))/(m1-m2)  # replace this with your calculation for y
    print(y)
    return y
